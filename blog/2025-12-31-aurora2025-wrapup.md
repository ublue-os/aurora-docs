...
flowchart TB
    subgraph oci["Bluefin OCI Containers"]
        ...
        aurora_common["<strong>@get-aurora-dev/common</strong><br/>Aurora Shared Configuration"]
    end
    
    subgraph base["Base Images"]
        ublue["<strong>Universal Blue</strong><br/>Base Image<br/>Shared with Bazzite KDE"]
    end
    
    subgraph images["Final Images"]
        aurora["aurora:stable"]
    end

    %% Aurora Common dependencies
    common --> aurora_common
    brew --> aurora_common
    artwork --> aurora_common
    
    %% Main Pipeline flow
    aurora_common --> ublue
    ublue --> aurora
    
    style oci fill:#708ee3,color:#fff
    style base fill:#4a69bd,color:#fff
    style images fill:#8a97f7,color:#fff
```
